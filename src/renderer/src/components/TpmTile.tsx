...
// TPM thresholds: <18 good, 18–25 monitor, >25 change now
function barColor(tpm: number): string {
  if (tpm < 18) return 'bg-emerald-500'
  if (tpm <= 25) return 'bg-amber-400'
  return 'bg-red-500'
}

function barLabel(tpm: number): string {
  if (tpm < 18) return 'Good'
  if (tpm <= 25) return 'Monitor'
  return 'Change Now'
}
    ...
    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 flex flex-col gap-4">
      ...
      <div className="space-y-2">
        <div className="w-full h-3 bg-slate-700 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-500 ${barColor(tpm)}`}
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      <p className="text-slate-500 text-xs">{barLabel(tpm)} range</p>
    </div>
  )
}
