...
export default function TemperatureTile() {
  const temperatureF = useOilStore((s) => s.temperatureF)
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 flex flex-col gap-4">
      ...
      <div className="flex items-end gap-2">
        <span
          className={`font-bold transition-colors duration-300 ${isHot ? 'text-red-400' : 'text-white'}`}
          style={{ fontSize: '3.5rem', lineHeight: 1 }}
        >
          {temperatureF}
        </span>
        <span className={`text-2xl font-medium pb-1 ${isHot ? 'text-red-400' : 'text-slate-400'}`}>
          °F
        </span>
      </div>
      {isHot && (
      ...
      <p className="text-slate-500 text-xs mt-auto">Target: {TARGET_F}°F</p>
    </div>
  )
}
